...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if(isset($username,$password,$remember) and !empty($username) and !empty($password))
	{
		$DNA = new DNA("../Robot/Config/Configuration.db");
		$AdminCfg = $DNA->GetAdminOptions();
		if($AdminCfg['Username'] == $username and $AdminCfg['Password'] == $password)
		{
			$Sess = new Session();
			@ $Sess->SessionSetData(array('login'=>md5($username + date('YMD')), 'session'=>sha1($password + date('YMD'))));
...
<html lang="en">
<head>
	<title>MioGram Community Admin</title>
	<meta charset="UTF-8">
